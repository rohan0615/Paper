...
data_dir = 'Data/' # path resoring training/validation/test data
...
if MODEL_TYPE == 1:
    MODEL_NAME = 'qp20~25'
    SELECT_QP_LIST = [20,22,25]
    EVALUATE_QP_THR_LIST = [21,23]
elif MODEL_TYPE == 2:
    MODEL_NAME='qp25~30'    
    SELECT_QP_LIST=[25,27,30]
    EVALUATE_QP_THR_LIST = [26,28]
elif MODEL_TYPE == 3:
    MODEL_NAME='qp30~35'    
    SELECT_QP_LIST=[30,22,35]
    EVALUATE_QP_THR_LIST = [31,33]
elif MODEL_TYPE == 4:
    MODEL_NAME='qp35~40'    
    SELECT_QP_LIST=[35,37,40]
    EVALUATE_QP_THR_LIST = [36,38]

DATA_SWITCH = 1 # choose which sets 0:full sets, 1:demo sets

def get_train_valid_test_sets(DATA_SWITCH):
    global TRAINSET, VALIDSET, TESTSET
    CORE_STRING='CUDepthExtractAllQPs'
    
    if DATA_SWITCH == 0: # full sets
        TRAINSET_SUFFIX = '_AI_CPIH_768_1536_2880_4928_nf425_Train_2446725.dat_shuffled'
        VALIDSET_SUFFIX = '_AI_CPIH_768_1536_2880_4928_nf25_Valid_143925.dat_shuffled'
        TESTSET_SUFFIX  = '_AI_CPIH_768_1536_2880_4928_nf50_Test_287850.dat_shuffled'
    elif DATA_SWITCH == 1: # demo sets
        
        TRAINSET_SUFFIX = '_AI_CPIH_768_1536_2880_4928_nf425_Train_20000.dat_shuffled'
        VALIDSET_SUFFIX = '_AI_CPIH_768_1536_2880_4928_nf25_Valid_10000.dat_shuffled'
        TESTSET_SUFFIX  = '_AI_CPIH_768_1536_2880_4928_nf50_Test_10000.dat_shuffled'
    
    elif DATA_SWITCH == 2: # choose other files if necessary
        TRAINSET_SUFFIX = '_AI_CPIH_768_1536_2880_4928_qp22_Train.dat'
        VALIDSET_SUFFIX = '_AI_CPIH_768_1536_2880_4928_qp22_Valid.dat'
        TESTSET_SUFFIX  = '_AI_CPIH_768_1536_2880_4928_qp22_Test.dat'

    TRAINSET = CORE_STRING + TRAINSET_SUFFIX
    VALIDSET = CORE_STRING + VALIDSET_SUFFIX
    TESTSET  = CORE_STRING + TESTSET_SUFFIX
...
def get_data_set(file_reader, read_bytes, is_loop=True, dtype=np.uint8 , is_show_stat=False):
    ...
    data_bytes=len(data)
    assert data_bytes % NUM_SAMPLE_LENGTH == 0 # check if condition is true or false. If false, assert will rise an exception message.
    num_samples = int(data_bytes / NUM_SAMPLE_LENGTH)
    
    data = data.reshape(num_samples, NUM_SAMPLE_LENGTH)
    
    images = data[:,0:4096].astype(np.float32) # image size is 64*64 = 4096
    images = np.reshape(images, [-1, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS])
    
    qps = np.random.choice(SELECT_QP_LIST,size=num_samples) # random choose a qp from qp list according the qp range
    ...
    if is_show_stat==True:
        ...
        count_list_select, _ = range_stat.feed_data_list(labels)
