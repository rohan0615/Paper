...
class FileReader(object):
    ...
    def read_data(self, read_bytes, isloop, dtype):
        ...
        if self._current_bytes + read_bytes <= self._max_bytes:
            buf = self._file_pointer.read(read_bytes) # read in size of 'read_bytes' characters
            data = numpy.frombuffer(buf, dtype=dtype) # interprete data inside buffer into an array of dtype
